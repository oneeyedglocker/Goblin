...
local function StatWidget(SL, parent, label, getter, subGetter)
  ...
  num:SetPoint("TOPLEFT", 12, -28); num:SetJustifyH("LEFT")
  num:SetFont(BODY_BOLD_FONT, 22); num:SetTextColor(1, 1, 1)
  local sub = Text(w, "LEFT", 11, false, COLORS_SUMMARY.dim)
  sub:SetPoint("BOTTOMLEFT", 12, 10); sub:SetPoint("BOTTOMRIGHT", -12, 10)
  w.Update = function(_, data)
...
function Widgets.trend(SL, parent)
  ...
  local num = w:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
  num:SetPoint("TOPLEFT", 12, -28); num:SetFont(BODY_BOLD_FONT, 22)
  local sub = Text(w, "LEFT", 11, false, COLORS_SUMMARY.dim)
  sub:SetPoint("BOTTOMLEFT", 12, 10); sub:SetPoint("BOTTOMRIGHT", -12, 10)
  w.Update = function(_, data)
    local history = data.history or {}
...
function Widgets.valueByLocation(SL, parent)
  ...
  for i, loc in ipairs(LOC_ORDER) do
    ...
    item:SetSize(160, 16)
    item:SetPoint("TOPLEFT", col * 170, -rowInLegend * 18)
  ...
  w.Update = function(_, data)
    ...
    for i, pc in ipairs(data.perCharacter) do
      ...
      if not row then
        ...
        row.name = Text(row, "LEFT", 11, true); row.name:SetPoint("LEFT", 4, 0); row.name:SetPoint("RIGHT", -160, 0); row.name:SetHeight(14); row.name:SetWordWrap(false)
        row.gold = Text(row, "RIGHT", 11, false, COLORS_SUMMARY.dim); row.gold:SetPoint("RIGHT", -80, 0); row.gold:SetWidth(70)
        row.value = Text(row, "RIGHT", 11, true, COLORS_SUMMARY.yellow); row.value:SetPoint("RIGHT", -4, 0); row.value:SetWidth(80)
        w.rows[i] = row
      end
